...
from analysis.linguistic_classifier import DGAClassifier, DGAClassifier_hmd
import pandas as pd
...
def feature(filename=time.strftime('%Y-%m-%d',time.localtime(time.time()))+'.txt',op=1):
        print("\t提取域名的特征值...")
        #数字占比
        file = open(parent_path+'/data_txt/'+filename,"r")
        lines=0
        domains={}
        for line in file:
                lines += 1
        ...
        for line in file:
                ...
                DGA_classifier = DGAClassifier_hmd(do)
