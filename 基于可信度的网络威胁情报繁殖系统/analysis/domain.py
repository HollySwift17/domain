from analysis.publicsuffix import PublicSuffixList
...
class Domain:
	def __init__(self, str_domain_name, experiment):
		self._domain_name = DomainName(str_domain_name)#.get_original_domain_name()
		...
		self._answers = None
		self._recently_registered = None
	...
	def set_ip_mappings(self, ips):
		self._ip_mappings = ips
	...
	def get_linguistic_feature_set(self):
		if self._linguistic_feature_set is None:
	...
	def get_bipartite_graph_feature_set(self):
		if self._bipartite_graph_feature_set is None:
...
class DomainName:
	def __init__(self, str_domain_name):
		self._original_domain_name = clean_string(str_domain_name) #lower 
	...
	def get_chosen_prefix(self):
		#_string = '.'.join(self._labels_list)
		return self._public_suffix
	...
	def get_number_of_labels(self):
		return len(list(self._labels_list))
