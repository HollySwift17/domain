from analysis.linguistic_features_extractors import MeaningfulWordsExtractor
from analysis.linguistic_features_extractors import NumericalCharactersExtractor
from analysis.linguistic_features_extractors import CharacterSetExtractor
from analysis.linguistic_features_extractors import NGramNormalityExtractor
from analysis.cluster import DomainClusterFileFactory
...
class FeatureSet:
	def __init__(self):
		...
		self._four_gram_normality_score = None
		self._five_gram_normality_score = None
		self._DGA_label = None

	def export_header(self):
		return 'meaningful_word_ratio, numerical_characters_ratio, one_gram_normality_score, two_gram_normality_score, three_gram_normality_score, four_gram_normality_score, five_gram_normality_score, DGA_label'
	...
	def __str__(self):
		...
		string += ', '
		string += str(self.get_four_gram_normality_score())
		string += ', '
		string += str(self.get_five_gram_normality_score())
		string += ', '
	...
	def get_meaningful_word_ratio(self):
		if self._meaningful_word_ratio is None:
	...
	def get_numerical_characters_ratio(self):
		if self._numerical_characters_ratio is None:
	...
	def get_character_set(self):
		if self._character_set is None:
	...
	def get_one_gram_normality_score(self):
		if self._one_gram_normality_score is None:
	...
	def get_two_gram_normality_score(self):
		if self._two_gram_normality_score is None:
	...
	def get_three_gram_normality_score(self):
		if self._three_gram_normality_score is None:
			raise Exception('The feature three_gram_normality_score was not set.')

		return self._three_gram_normality_score

	def set_four_gram_normality_score(self, feature):
		self._four_gram_normality_score = feature

	def get_four_gram_normality_score(self):
		if self._four_gram_normality_score is None:
			raise Exception('The feature four_gram_normality_score was not set.')

		return self._four_gram_normality_score

	def set_five_gram_normality_score(self, feature):
		self._five_gram_normality_score = feature

	def get_five_gram_normality_score(self):
		if self._five_gram_normality_score is None:
			raise Exception('The feature five_gram_normality_score was not set.')

		return self._five_gram_normality_score

	def set_DGA_label(self, label):
		self._DGA_label = label

	def get_DGA_label(self):
		if self._DGA_label is None:
			return 'unknown'
	...
	def get_mahalanobis(self):
		if self._mahalanobis_distance is None:
...
class FeaturesExtractor:
	...
	def compute_feature_set(self, dictionary=None):
		...
		features_set.set_four_gram_normality_score(n_gram_normality_extractor.normality_score(4))
		features_set.set_five_gram_normality_score(n_gram_normality_extractor.normality_score(5))
...
class DGAClassifier:
	...
	def classify(self, strict=True):
		...
		four_gram_normality_score = feature_set.get_four_gram_normality_score() #>>1
		five_gram_normality_score = feature_set.get_five_gram_normality_score() #>>1
...
class DGAClassifier_bmd:
	...
	def classify(self, strict=True):
		...
		distance = linguistic_normality.compute_distance_bmd(self._domain)
...
class DGAClassifier_hmd:
	...
	def classify(self, strict=True):
		...
		distance = linguistic_normality.compute_distance_hmd(self._domain)
...
class LinguisticNormality(BaseException):
	__instance = None
...
#alexa100.txt
		cluster_factory = DomainClusterFileFactory('analysis/alexa100.txt', False)
		...
		four_gram_normality_score_list = list()
		five_gram_normality_score_list = list()
		...
		for domain in normal_cluster:
			...
			four_gram_normality_score_list.append(feature_set.get_four_gram_normality_score())
			five_gram_normality_score_list.append(feature_set.get_five_gram_normality_score())
	...
	def compute_distance(self, domain):
		...
		###if current_sample[i]  is bigger than centroid[i],then it must be no_dga.
