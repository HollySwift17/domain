from analysis.tools import clean_string
from analysis.domain import Domain
from analysis.database import DatabaseInterface
from analysis.cluster_descriptor import BipartiteGraphDescriptor
from analysis.timeseries import TimeseriesToolbox
from analysis.tools import AutonomousSystemDatabase
from analysis.tools import colored_label
import random
import numpy
import hashlib
...
def merge_clusters(cluster_a, cluster_b):
    old_ids = cluster_a.get_identifier() + cluster_b.get_identifier()
    m = hashlib.new('ripemd160')
    m.update(old_ids)
    name = m.hexdigest()
    new_cluster = DomainCluster(name)
    new_cluster._domain_list.extend(cluster_a.get_domains_list())
    new_cluster._domain_list.extend(cluster_b.get_domains_list())
...
class DomainClusterFileFactory:
    def __init__(self, file_path, two_columns_format, cluster_identifier="cluster_0"):
        self._clusters = dict()

        with open(file_path, 'r') as lines:
            if not two_columns_format:
                self._clusters[cluster_identifier] = DomainCluster(cluster_identifier)
            ...
            for line in lines:#############################################
                ...
                if two_columns_format:
                    ...
                    self._clusters[cluster_identifier].add_domain(Domain(line, None))
...
class DomainCluster:
    ...
    def get_size(self):
        if self._domain_list is not None:
            return len(self._domain_list)
