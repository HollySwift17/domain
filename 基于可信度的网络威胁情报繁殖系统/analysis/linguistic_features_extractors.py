from analysis.tools import Dictionary
import re
import math
...
class NumericalCharactersExtractor:
	...
	def characters_number(self):
		...
		domain_name = self._str_domain_name
		filtered_str_domain_name = regex.sub('', domain_name)
	...
	def normality_score(self, n, input_dictionary=None):
		...
		for n_gram in get_n_grams(n, self._str_domain_name):
			if n == 1:
				...
				current_count = dictionary.four_gram_count(n_gram)
			elif n == 5:
				current_count = dictionary.five_gram_count(n_gram)
				
			else:
				raise Exception('In NGramAnomalyExtractor n should be 1, 2, 3, 4 or 5')
